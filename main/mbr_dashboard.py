...
def load_month(filepath: str):
    ...
    # Positives descending first, negatives ascending (most negative last)
    _wip_pos = sorted([p for p in wip_projects if p["wip_tl"] >= 0], key=lambda x: x["wip_tl"], reverse=True)
    _wip_neg = sorted([p for p in wip_projects if p["wip_tl"] < 0],  key=lambda x: x["wip_tl"])
    wip_projects = _wip_pos + _wip_neg
